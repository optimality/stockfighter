...
func (t *Trader) Run() {
	go func() {
		c, err := t.Client.GetQuoteSocket(t.Venue, t.Account)
		if err != nil {
			panic(err)
		}
		for q := range c {
			t.UpdatePriceEstimate(q.Quote.Last)
		}
	}()

	go func() {
		c, err := t.Client.GetExecutionSocket(t.Venue, t.Account)
		if err != nil {
			panic(err)
		}
		for e := range c {
	...
	for {
		...
		/*
			nav := t.Position*t.PriceEstimate + t.Cash
			fmt.Printf("Cash: $%v Position: %v NAV: $%v "+
				...
				t.Ask.OriginalQty-t.Ask.TotalFilled, float64(t.Ask.Price)/100, float64(t.PriceEstimate)/100)
		*/
	}
}
